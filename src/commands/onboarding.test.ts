...
test("projects valid categories and products while dropping native fields", async () => {
  ...
  const product = { product_id: 8, sku: "BEL-1", name: "Accessory belt", category_id: 3, category_name: "Belts", catalog_unit_price_centavos: 4500, available_quantity: 6, active: true, hidden: true };
  ...
  assert.deepEqual(await createCreateProductCommand(async () => ({ kind: "success", ...product }))({ sku: "BEL-1", name: "Accessory belt", category_id: 3, catalog_unit_price_centavos: 4500, opening_quantity: 6, attribute_values: [] }), { kind: "success", product_id: 8, sku: "BEL-1", name: "Accessory belt", category_id: 3, category_name: "Belts", catalog_unit_price_centavos: 4500, available_quantity: 6, active: true });
});
...
test("rejects malformed onboarding data atomically and checks numeric transport", async () => {
  ...
  const malformedProduct = createCreateProductCommand(async () => ({ kind: "success", product_id: 1, sku: "x", name: "x", category_id: 1, category_name: "x", catalog_unit_price_centavos: 1.5, available_quantity: 1, active: true }));
  assert.deepEqual(await malformedProduct({ sku: "x", name: "x", category_id: 1, catalog_unit_price_centavos: 1, opening_quantity: 1, attribute_values: [] }), { kind: "error", code: "persistence_failure", message: "The product could not be persisted." });
});
...
test("maps recognized and malformed onboarding errors to fixed safe messages", async () => {
  ...
  assert.deepEqual(await createCreateProductCommand(async () => ({ kind: "error", code: "unknown", message: "native" }))({ sku: "x", name: "x", category_id: 1, catalog_unit_price_centavos: 1, opening_quantity: 1, attribute_values: [] }), { kind: "error", code: "persistence_failure", message: "The product could not be persisted." });
