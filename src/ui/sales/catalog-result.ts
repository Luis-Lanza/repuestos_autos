...
export function catalogResultDetails(product: ProductSearchResult): string {
  return `${product.sku} — ${product.name} — ${product.category_name} — minimum ${formatBolivianos(product.minimum_unit_price_centavos)} (${product.available_quantity} in stock)`;
}
