...
export type DraftLine = {
  ...
  list_price_centavos: number; minimum_price_centavos: number;
...
export function createSaleFlow(state: SaleState, action: SaleAction): SaleState {
  switch (action.type) {
    ...
    case "add_product":
      ...
      { const listPrice = Number.isSafeInteger(action.product.list_price_centavos) ? action.product.list_price_centavos : action.product.catalog_unit_price_centavos;
        const minimumPrice = Number.isSafeInteger(action.product.minimum_sale_price_centavos) ? action.product.minimum_sale_price_centavos : listPrice;
        return { ...resetIntent(state), lines: [...state.lines, { product_id: action.product.product_id, sku: action.product.sku, product_name: action.product.name, quantity: 1, captured_unit_price_centavos: action.product.catalog_unit_price_centavos, captured_revision: action.product.revision, list_price_centavos: listPrice, minimum_price_centavos: minimumPrice, final_price_input: formatBsInput(listPrice) }], feedback: null }; }
    case "remove_product": {
