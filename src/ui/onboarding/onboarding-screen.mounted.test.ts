...
test("renders shared Spanish panels and preserves Bs/whole-unit payload", async () => {
  let request: unknown;
  mockIPC((command, payload) => command === "list_categories_command" ? success() : command === "create_product_command" ? (request = payload, { kind: "success", product_id: 2, sku: "FIL-1", name: "Filtro", category_id: 1, category_name: "Filtros", list_price_centavos: 12550, minimum_sale_price_centavos: 10000, available_quantity: 3, active: true }) : undefined);
  ...
  await user.type(screen.getByRole("textbox", { name: "SKU" }), "FIL-1"); await user.type(screen.getByRole("textbox", { name: "Nombre del producto" }), "Filtro"); await user.type(screen.getByRole("textbox", { name: "Precio de lista (Bs)" }), "125,50"); await user.type(screen.getByRole("textbox", { name: "Precio mínimo de venta (Bs)" }), "100,00"); await user.type(screen.getByRole("spinbutton", { name: "Stock inicial (unidades enteras)" }), "3"); await user.type(screen.getByRole("textbox", { name: "Marca" }), "ACDelco"); await user.click(screen.getByRole("button", { name: "Crear producto" }));
  assert.deepEqual(request, { request: { sku: "FIL-1", name: "Filtro", category_id: 1, list_price_centavos: 12550, minimum_sale_price_centavos: 10000, opening_quantity: 3, attribute_values: [{ definition_id: 10, value: "ACDelco" }] } }); assert.equal((await screen.findByRole("status")).textContent, "Producto creado: FIL-1. Stock inicial: 3 unidades.");
});
...
test("focuses the first invalid product field", async () => {
  ...
  await user.type(screen.getByRole("textbox", { name: "SKU" }), "FIL-1"); await user.type(screen.getByRole("textbox", { name: "Nombre del producto" }), "Filtro"); await user.type(screen.getByRole("textbox", { name: "Precio de lista (Bs)" }), "0"); await user.type(screen.getByRole("spinbutton", { name: "Stock inicial (unidades enteras)" }), "0"); await user.click(screen.getByRole("button", { name: "Crear producto" }));
  assert.equal(document.activeElement, screen.getByRole("textbox", { name: "Precio de lista (Bs)" }));
});
test("ignores product completion after unmount", async () => {
  ...
  await user.type(screen.getByRole("textbox", { name: "SKU" }), "FIL-1"); await user.type(screen.getByRole("textbox", { name: "Nombre del producto" }), "Filtro"); await user.type(screen.getByRole("textbox", { name: "Precio de lista (Bs)" }), "1"); await user.type(screen.getByRole("textbox", { name: "Precio mínimo de venta (Bs)" }), "1"); await user.type(screen.getByRole("spinbutton", { name: "Stock inicial (unidades enteras)" }), "1"); await user.type(screen.getByRole("textbox", { name: "Marca" }), "ACDelco"); await user.click(screen.getByRole("button", { name: "Crear producto" })); view.unmount(); resolve({ kind: "success", product_id: 2, sku: "FIL-1", name: "Filtro", category_id: 1, category_name: "Filtros", list_price_centavos: 100, minimum_sale_price_centavos: 100, available_quantity: 1, active: true }); await Promise.resolve(); assert.equal(document.body.textContent?.includes("Producto creado"), false);
});
