...
export interface CatalogEditForm { name: string; sku?: string; list_price_centavos?: string; minimum_sale_price_centavos?: string; attribute_values: Record<number, string>; }
...
export function formForCatalogDetail(detail: CatalogMetadataDetail): CatalogEditForm { return detail.target === CATALOG_TARGET.CATEGORY ? { name: detail.name, attribute_values: {} } : { sku: detail.sku, name: detail.name, list_price_centavos: formatCatalogBs(detail.list_price_centavos), minimum_sale_price_centavos: formatCatalogBs(detail.minimum_sale_price_centavos), attribute_values: Object.fromEntries(detail.attribute_values.map((value) => [value.definition_id, value.value])) }; }
export function fieldErrorsForCatalogEdit(detail: CatalogMetadataDetail, form: CatalogEditForm): CatalogEditFieldErrors {
  ...
  if (parseCatalogBs(form.list_price_centavos ?? "") === null) errors.list_price_centavos = "Ingresá un precio de lista válido en Bs.";
      if (parseCatalogBs(form.minimum_sale_price_centavos ?? "") === null) errors.minimum_sale_price_centavos = "Ingresá un precio mínimo de venta válido en Bs.";
      else if (parseCatalogBs(form.list_price_centavos ?? "") !== null && parseCatalogBs(form.minimum_sale_price_centavos ?? "")! > parseCatalogBs(form.list_price_centavos ?? "")!) errors.minimum_sale_price_centavos = "El precio mínimo no puede superar el precio de lista.";
...
export function createCatalogEditRequest(detail: CatalogMetadataDetail, form: CatalogEditForm): CatalogEditInput | null {
  ...
  return { target: detail.target, entity_id: detail.entity_id, expected_revision: detail.revision, sku: form.sku ?? "", name: form.name, list_price_centavos: parseCatalogBs(form.list_price_centavos ?? "") as number, minimum_sale_price_centavos: parseCatalogBs(form.minimum_sale_price_centavos ?? "") as number, attribute_values: values.filter((value) => detail.attribute_definitions.find((field) => field.definition_id === value.definition_id)?.required || value.value !== "") };
}
